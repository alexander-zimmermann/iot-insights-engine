...
_SITE = Site(
    ...
    planes=(Plane(key="West", inverter_id=1, tilt=17, azimuth=129, kwp=6.435),),
)
...
def _params() -> dict[str, str]:
    return forecast_weather._build_params(_settings(), _SITE)


def test_build_params_shape() -> None:
    # The site's location; the request itself is in UTC, so the site's
    # timezone plays no part here.
    params = _params()
    assert params["latitude"] == "50.626"
    assert params["longitude"] == "6.024"
...
def test_fetch_hourly_parses() -> None:
    ...
    with respx.mock(assert_all_called=True) as router:
        ...
        hourly = forecast_weather._fetch_hourly(base, _params())
...
def test_fetch_hourly_rejects_missing_metric() -> None:
    ...
    with respx.mock(assert_all_called=True) as router:
        ...
        with pytest.raises(ValueError, match="missing requested metric 'cloud_cover'"):
            forecast_weather._fetch_hourly(base, _params())
